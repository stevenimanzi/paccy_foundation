...
export default async function AdminPage(){
  ...
  const db=getDb();
  const [volunteerRows,donationRows,messageRows,contentRows,activityRows,userRows,visitorRows,userCountRows,viewCountRows,activeSessionRows,galleryRows,monthlyTrafficRows]=await Promise.all([
    ...
    db.select({month:sql<string>`SUBSTRING(${pageViews.createdAt},1,7)`,visits:count(),visitors:sql<number>`COUNT(DISTINCT ${pageViews.visitorId})`}).from(pageViews).groupBy(sql`SUBSTRING(${pageViews.createdAt},1,7)`).orderBy(sql`SUBSTRING(${pageViews.createdAt},1,7)`),
  ]);
  const trafficMap=new Map(monthlyTrafficRows.map(row=>[row.month,{visits:Number(row.visits),visitors:Number(row.visitors)}]));
  const trafficData=Array.from({length:12},(_,index)=>{const date=new Date();date.setUTCDate(1);date.setUTCMonth(date.getUTCMonth()-(11-index));const month=date.toISOString().slice(0,7),values=trafficMap.get(month)??{visits:0,visitors:0};return{month,label:date.toLocaleDateString("en",{month:"short"}),...values}});
  ...
  <AdminToolbar/>
  <section className="admin-stats" id="overview"><article><span>Platform users</span><strong>{userCountRows[0]?.value??0}</strong></article><article><span>Active sessions</span><strong>{activeSessionRows[0]?.value??0}</strong></article><article><span>Website visits</span><strong>{viewCountRows[0]?.value??0}</strong></article><article><span>Volunteer applications</span><strong>{volunteerRows.length}</strong></article><article><span>Donation requests</span><strong>{donationRows.length}</strong></article><article><span>Unread messages</span><strong>{messageRows.filter(x=>x.status==="unread").length}</strong></article><TrafficChart data={trafficData}/></section>
  <AdminSection id="users" title="Platform users"><AddUser/><RecordTable rows={userRows} type="user" columns={["name","email","role","status","lastLoginAt"]}/><h3 className="admin-subtitle">Recent website visits</h3><RecordTable rows={visitorRows} type="visitor" columns={["visitorId","path","createdAt"]}/></AdminSection>
  ...
  <AdminSection id="messages" title="Contact messages"><div className="admin-table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Message</th><th>Status</th><th>Action</th></tr></thead><tbody>{messageRows.length?messageRows.map(row=><tr key={row.id}><td>{row.name}</td><td><a href={`mailto:${row.email}`}>{row.email}</a></td><td className="message-copy">{row.message}</td><td><select data-record-status data-type="message" data-id={String(row.id)} defaultValue={row.status}><option>unread</option><option>read</option><option>replied</option></select></td><td><MessageReply messageId={row.id} name={row.name} email={row.email} originalMessage={row.message}/></td></tr>):<tr><td colSpan={5}>No messages yet.</td></tr>}</tbody></table></div></AdminSection>
...
function RecordTable({rows,type,columns}:{rows:Record<string,unknown>[];type:string;columns:string[]}){return <div className="admin-table-wrap"><table><thead><tr>{columns.map(c=><th key={c}>{c}</th>)}</tr></thead><tbody>{rows.length?rows.map((row)=><tr key={String(row.id)}>{columns.map(c=><td key={c}>{c==="status"&&type!=="activity"?<select data-record-status data-type={type} data-id={String(row.id)} defaultValue={String(row[c])}>{type==="user"&&<><option>active</option><option>suspended</option></>}<option>new</option><option>pending</option><option>reviewed</option><option>approved</option><option>completed</option><option>declined</option><option>unread</option><option>read</option><option>replied</option></select>:String(row[c]??"—")}</td>)}</tr>):<tr><td colSpan={columns.length}>No records yet.</td></tr>}</tbody></table></div>}
